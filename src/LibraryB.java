...
public class LibraryB implements Library{
    ...
    public int giveBack(String bookDetails, String memberType, String ID, String time) {
        ...
        String translator;
        if (details.length==3)
            translator = details[2];
        else
            translator = null;
        Book book = searchInBookList(details[0],details[1],translator);
        if (findBookFromBookList(details[0],details[1],translator)==1) {
            if (!(book.equals(null))) {
                if (memberType.equalsIgnoreCase("Student")) {
                    Student student = Main.searchStudent(ID);
    ...
    public static int searchInLibrary(Book book) {
        ...
        for (Book b : bookList)
            if (b.authorName.equals(book.authorName)) {
                firstSearch.add(b);
            }
    ...
    public static Book searchInBookList (String authorName, String year, String translator){
        List<Book> firstSearch = new ArrayList<Book>();
        List<Book> secondSearch = new ArrayList<Book>();
        for (Book b : bookList)
            if (b.authorName.equals(authorName)) {
                firstSearch.add(b);
            }
        for (Book b2 : firstSearch)
    ...
    public static int findBookFromBookList (String authorName, String year, String translator){
        boolean found=false;
        List<Book> firstSearch = new ArrayList<Book>();
        List<Book> secondSearch = new ArrayList<Book>();
        for (Book b : bookList)
